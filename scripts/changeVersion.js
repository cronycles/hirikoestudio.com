...
function changeProjectVersion(newVersion) {
  ...
  return changeEnvFilesVersion(newVersion);
}
...
function setConfigFileVersion(newVersion) {
  ...
  fs.readFile(fileName, 'utf8', function (err, data) {
    ...
    var result = data.replace(/('version'\s*=>\s*env\('APP_VERSION',\s*')(.*)('\),)/g, "$1" + newVersion + "$3");
...
function changeEnvFilesVersion(newVersion) {
  var fileList = ["./.env", "./.env.local", "./.env.development", "./.env.production"];
    ...
    fs.readFile(fileName, 'utf8', function (err, data) {
      if (err) {
        if (err.code != 'ENOENT') {
          ...
          //file non trovato ma non é un errore
          return console.log(fileName);
          return true;
        }
      }
      var result = data.replace(/(APP_VERSION=\s*)(.*)/g, "$1" + newVersion);
